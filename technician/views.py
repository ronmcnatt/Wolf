...
def seed_customers(request):
    ...
    try:
        for s in SAMPLE:
            _, created = Customer.objects.get_or_create(business_name=s['business_name'], defaults=s)
            results.append({'name': s['business_name'], 'created': created})
        return JsonResponse({'ok': True, 'customers': results})
